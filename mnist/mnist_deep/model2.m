...
conv1_kernel_size = 5;
conv1_kernel_num = 32;
conv1_kernel = 0.1 * randn(conv1_kernel_size,conv1_kernel_size,image_channel,conv1_kernel_num);
conv1_biases = 0.1 * ones(1,conv1_kernel_num);

conv2_kernel_size = 5;
conv2_kernel_num = 64;
conv2_kernel = 0.1 * randn(conv2_kernel_size,conv2_kernel_size,conv1_kernel_num,conv2_kernel_num);
...
for j = 1:pass
    for i = 1:train_size
	data = reshape(train_dataset(:,:,i),image_size,image_size,1);
	...
	[loss,logits,derive_fc2_biases] = softmax_cross_entropy(fc2_out,train_label(:,i),num_classes);
	...
	derive_conv2_kernel = kernel_derive(conv1_pooling,conv2_theta,conv2_kernel_size);
	...
	[arg,max_logits] = max(logits);
	[arg,max_label] = max(train_label(:,i));
	accuracy = accuracy + (max_logits == max_label);
	if mod(i,100) == 0
	    loss_sum
